...
export class IncomeExpense {
    constructor() {
        $('#income-expense').addClass('active');

        $('#createIncome').on('click', function () {
            localStorage.setItem('typeRecord', 'income');
            location.href = '#/create-income-expense';
        });
        $('#createExpense').on('click', function () {
            localStorage.setItem('typeRecord', 'expense');
            location.href = '#/create-income-expense';
        });

        Popup.getPopup('Вы действительно хотите удалить операцию?');
        FilterTools.getFilterTools(this.showOperations.bind(this));
    }

    showOperations() {
        $('#operations').html('');
        FilterTools.operations.forEach(item => {
            let operation = `<tr>
                                <th scope="row" class="ps-4 fw-normal">
                                    <div class="row">
                                        <div class="col-1 ps-0 pe-1 fw-bold">${item.id}</div>
                                        <div class="col-2 ${item.type === 'expense' ? 'text-danger">расход' : 'text-success">доход'}</div>
                                        <div class="col-2">${item.category}</div>
                                        <div class="col-1">${item.amount + config.currency}</div>
                                        <div class="col-2">${CreateSpecialElement.convertToFormatDate(item.date, 'YYYYMMDD', '-', 'DDMMYYYY', '.')}</div>
                                        <div class="col-3">${item.comment}</div>
                                        <div class="col-1">
                                            <a href="javascript:void(0)" id="delete-${item.id}" class="me-2 text-decoration-none delete-operation">
                                            ...
                                            <a href="javascript:void(0)" id="edit-${item.id}" class="text-decoration-none edit-operation">
            ...
            $('#operations').html((index, oldHtml) => oldHtml + operation);
        })

        $('.edit-operation').click(function () {
            localStorage.setItem('idRecord', this.id.split('-').pop());
            location.href = '#/edit-income-expense';
        })

        $('.delete-operation').click(function () {
            Popup.showPopup();
            Popup.addEventDelete(this.id.split('-').pop(), IncomeExpense.deleteOperation);
        })
    }
